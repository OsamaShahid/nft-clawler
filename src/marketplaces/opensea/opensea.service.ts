...
export class OpenseaService {
  ...
  async getAssetFromOpenSeaService(
    tokenId: string,
  ...
  async enumerateTransactionsByWalletOnEthereum(wallet: string) {
    let offset = 0;
    const limit = 50;
    ...
    while (true) {
      const url = `https://api.opensea.io/api/v1/events?account_address=${wallet}&offset=${offset}&limit=${limit}`;
      const options = {
        headers: { "X-API-KEY": "025ccfe1f0dc46cba0d80d8fba31b720" }
      };
      const data = this.httpService.get(url, options).pipe(
      ...
      const opensea = await lastValueFrom(data);
      allEvents = [...allEvents, ...opensea.asset_events];
      offset += limit;

      console.log(allEvents.length);

      if (opensea.asset_events.length < limit) {
        break;
      }
    }
